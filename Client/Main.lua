	...
	while true do
		...
		if ESX ~= nil then
			...
			if PlayerJobInfo ~= nil then
				if PlayerJobInfo.name == "pc-technician" then
				...
				if CurrentJob.Enabled == false then
					...
					if Vdist2(PlayerCoords, JobCoords) <= 1.5 and PlayerVehicle == 0 then
						...
						if IsControlJustPressed(1, 51) then
							...
							Animation = {
								animationDictionary = "anim@heists@prison_heiststation@cop_reactions",
							...
							--[[exports.rprogress:Custom({
								...
								--playAnim('anim@heists@prison_heiststation@cop_reactions', 'cop_b_idle', 5000)
								Citizen.Wait(10500)
								RemoveBlip(CurrentJob.Blip)
								DeleteWaypoint()
								CurrentJob = nil
